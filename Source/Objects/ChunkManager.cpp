	...
	if (sqrt(x * x + y * y + z * z) > 1.0f) {
		...
		for (int dx = -DISTANCE_TO_LOAD; dx <= DISTANCE_TO_LOAD; dx++)
		{
			...
			for (int dz = -dzMax; dz <= dzMax; dz++)
			{
				...
				{
					std::shared_lock<std::shared_mutex> lock(m_chunksMapMutex);
...
//---------------------------------------END_LOAD_NEW_CHUNKS---------------------------------
		...
		{
			std::shared_lock<std::shared_mutex> chunksMapLock(m_chunksMapMutex);
...
/// <summary>
/// init load chunk thread and unload chunk thread
/// 
/// </summary>
void ChunkManager::initWorkerThread()
{
	std::thread loadChunkThread([this] {
		...
		while (true)
		{
			...
			while (!m_chunkLoadQueue.empty())
			{
				...
				{
					std::unique_lock<std::shared_mutex> chunksLock(m_chunksMapMutex);
	...
	std::thread unloadChunkThread([this] {
		...
		while (true)
		{
			...
			while (!m_chunkUnloadQueue.empty())
			{
				...
				{
					std::unique_lock<std::shared_mutex> chunkLock(m_chunksMapMutex);
					if (m_chunks.find(chunkPos) != m_chunks.end()) {
...
void ChunkManager::updateChunkMap()
{
}
